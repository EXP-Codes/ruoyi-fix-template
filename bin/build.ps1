...
$TARGET_DIR = Join-Path -Path $APPBASE_DIR -ChildPath "target"
...
if ($REBUILD -eq "--recompile") {
    ...
    # 打包依赖工程(父工程)
    Write-Host "precompiled ${APP_DIR} pom ..."
    ...
    # 打包基础后端代码
    Write-Host "precompiled ${APPBASE_DIR} code ..."
    Set-Location -Path ${APPBASE_DIR}
